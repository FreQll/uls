...
void print_total(t_list *list) {
    ...
    for (t_list *i = list; i != NULL; i = i->next) {
        long stats = stat(i->data, &st);
        if (stats != -1) total += st.st_blocks;
        //if (stats != -1) total += (st.st_size + block_size - 1) /  block_size;
    }
...
void print_linked_links(char *name, int spaces) {

    struct stat fileStat;
	stat(name, &fileStat);

    if (get_num_digits(fileStat.st_nlink) < spaces) {
        for (int i = 0; i < spaces - get_num_digits(fileStat.st_nlink); i++) {
            mx_printchar(' ');
        }
    }
...
void print_file_time(char *name) {
    struct stat fileStat;
	stat(name, &fileStat);
    char *time = convert_time(fileStat.st_mtime);
    mx_printstr(time);
    mx_strdel(&time);
